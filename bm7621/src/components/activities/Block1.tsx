...
export function Block1Panel() {
  ...
  const checkA2 = () => {
    ...
    const pts = Math.min(5, Math.ceil(correct * 1.25))
    updateScore('a2', pts)
    updateResponse({ a2_intents: intents })
    setA2Result(`${correct}/${intentKeys.length} correct → ${pts} points`)
  }
  ...
  const handleSim = (vals: (number | null)[]) => {
    ...
    // also register sim score in activity scores
  ...
  return (
    <div>
      ...
      <ActivityCard number={1} title="Human Seed List" subtitle="Generate keywords a real customer would search" points={scores.a1?.points || 0}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input className="form-input mb-2" placeholder="e.g. running shoes" value={st1} onChange={e => { setSt1(e.target.value); scoreA1({ st1: e.target.value }) }} />
            <input className="form-input" placeholder="e.g. women's fashion" value={st2} onChange={e => { setSt2(e.target.value); scoreA1({ st2: e.target.value }) }} />
          </div>
          <div>
            <label className="form-label">Long-Tail Keywords (3+ words)</label>
            <input className="form-input mb-2" placeholder="e.g. cheap flights from London to Berlin" value={lt1} onChange={e => { setLt1(e.target.value); scoreA1({ lt1: e.target.value }) }} />
            <input className="form-input" placeholder="e.g. best running shoes for flat feet" value={lt2} onChange={e => { setLt2(e.target.value); scoreA1({ lt2: e.target.value }) }} />
          </div>
        </div>
      </ActivityCard>
      ...
      <ActivityCard number={2} title="Intent Mapping" subtitle="Match each keyword to its search intent" points={scores.a2?.points || 0}>
        <Alert type="info">🎯 Assign each keyword to the correct intent type. Auto-scored against the answer key.</Alert>
        <div className="space-y-3 mb-4">
          {intentKeys.map((item, i) => (
            <div key={i} className="flex items-center gap-3">
              <div className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2 text-sm font-mono text-slate-700">
                {item.kw}
              </div>
              <select
        ...
        <button className="btn-success btn-sm" onClick={checkA2}>Check Answers</button>
        {a2Result && (
          <div className="alert-success mt-3">{a2Result}</div>
        )}
      </ActivityCard>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
          {serpItems.map(item => {
            ...
            return (
              <label
                key={item}
                className={`flex items-center gap-2 p-3 border rounded-lg cursor-pointer text-sm transition-all ${
                  checked ? 'border-brand-400 bg-brand-50 text-brand-700' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'
                }`}
              >
                <input
                  type="checkbox"
      ...
      <ActivityCard number="SIM" title="Search Lab — Simulator" subtitle="Enter individual student scores from Search Lab" points={scores.sim1?.points || 0} isSimulator>
        ...
        {sim1 && sim1.average !== null && (
          <div className="alert-success mt-3">
            Average: <strong>{sim1.average.toFixed(1)}</strong> → <strong>{sim1.points} workshop points</strong>
          </div>
        )}
      </ActivityCard>
    </div>
